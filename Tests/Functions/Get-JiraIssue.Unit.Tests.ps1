...
Describe "Get-JiraIssue" -Tag 'Unit' {
    ...
    BeforeAll {
        ...
        $jqlEscaped = [System.Web.HttpUtility]::UrlEncode($jql)
        $response = @'
{
    "expand": "schema,names",
    "startAt": 0,
...
'@
        ...
        function Get-TestJiraFilter {
            ...
            [PSCustomObject]@{
                ...
                Id         = $Id
                SearchUrl  = "https://jira.example.com/rest/api/2/filter/$Id"
            }
        }

        Mock Get-JiraUser -ModuleName JiraPS {
            Get-TestJiraUser -Name 'username'
        }

        Mock Get-JiraFilter -ModuleName JiraPS {
            Get-TestJiraFilter -Id 12345
        }
