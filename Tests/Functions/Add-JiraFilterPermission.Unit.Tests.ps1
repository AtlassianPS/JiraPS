...
"@
        ...
        # Helper function for creating filter objects
        function New-TestJiraFilter {
            param(
        ...
        Mock Get-JiraFilter -ModuleName JiraPS {
            return (New-TestJiraFilter -Id $Id)
        }
    ...
    Context "Behavior Testing" {
        ...
        It "Adds share permission to Filter Object" {
            { Add-JiraFilterPermission -Filter (New-TestJiraFilter -Id 12844) -Type "Global"} |
                Should -Not -Throw

            $jiraMethodMockParams = @{
                Invoke = $true
            ...
            Should @jiraMethodMockParams

            $convertFilterMockParams = @{
                Invoke = $true
                CommandName = "ConvertTo-JiraFilter"
                ModuleName = "JiraPS"
                Exactly = $true
                Times = 1
            }
            Should @convertFilterMockParams
        }
    ...
    Context "Input testing" {
        It "requires -Filter to be a JiraPS.Filter" {
            { Add-JiraFilterPermission -Filter 1 -Type "Global" } | Should -Throw -ExpectedMessage "Cannot bind argument to parameter 'Filter'*"
            { Add-JiraFilterPermission -Filter "lorem" -Type "Global" } | Should -Throw -ExpectedMessage "Cannot bind argument to parameter 'Filter'*"
            { Add-JiraFilterPermission -Filter (Get-Date) -Type "Global" } | Should -Throw -ExpectedMessage "Cannot bind argument to parameter 'Filter'*"

            { Add-JiraFilterPermission -Filter (New-TestJiraFilter -Id 1) -Type "Global" } | Should -Not -Throw
        }

        It "allows a JiraPS.Filter to be passed over the pipeline" {
            { New-TestJiraFilter -Id 1 | Add-JiraFilterPermission -Type "Global" } | Should -Not -Throw
        }

        It "can process multiple Filters" {
            $filters = 1..5 | ForEach-Object { New-TestJiraFilter -Id 1 }
        ...
        It "can process multiple FilterIds" {
            ...
            $assertMockParams.CommandName = "Invoke-JiraMethod"
            $assertMockParams.Times = 5
            Should @assertMockParams
        }
