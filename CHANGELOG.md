...
- `Remove-JiraVersion` now targets the canonical `/rest/api/2/version/{id}` delete endpoint, retries short-lived HTTP 405 responses, and falls back to `/rest/api/2/version/{id}/removeAndSwap` (with an explicit empty JSON body) when Jira keeps rejecting DELETE with method-not-allowed. The internal version-delete calls now run with `-ErrorAction Stop` so the retry/fallback logic is deterministic even when callers use the default error preference. This stabilizes Jira Data Center runs where immediate version deletion can be rejected despite a valid version id.
