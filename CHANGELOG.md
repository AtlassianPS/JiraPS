...
- Narrowed the `Lint` task's `Invoke-ScriptAnalyzer` scope from the full project root to the actual code roots (`JiraPS/`, `Tests/`, `Tools/`, `JiraPS.build.ps1`). PSSA previously recursed from `$env:BHProjectPath` and then discarded `Release/*` findings client-side, which forced it to parse the ~130 duplicated `.ps1` / `.psm1` / `.psd1` files under `Release/` after every `Build`. Listing the source roots explicitly skips that work entirely. The client-side `Where-Object` filter (and the now-unused `${/}` directory-separator helper) are gone with it. Measured on macOS with a populated `Release/`: cold `Invoke-Build -Task Lint` drops from ~17.7 s to ~16.0 s; warm runs drop from ~3.2 s to ~1.8 s (-43 %). The win is larger on Windows where AV scanning every file reparse adds disproportionate overhead.
- Made `GenerateExternalHelp` incremental via Invoke-Build's `-Inputs` / `-Outputs`. The task previously reparsed every `docs/<locale>/commands/*.md` and `docs/<locale>/about_*.md`, re-imported `Microsoft.PowerShell.PlatyPS`, and re-spliced the MAML XML on every build, even when nothing under `docs/` had changed. It is now declared with `-Inputs` (the markdown files) and `-Outputs` (the per-locale `<ProjectName>-help.xml` plus one `about_*.help.txt` per `about_*.md`); Invoke-Build skips the body entirely when every output exists and is newer than every input. The `Clean` task no longer wipes `JiraPS/<locale>/` (those files are now an inter-build cache; `Release/` is still wiped from scratch every build, so end users are unaffected). To prevent stale artifacts from surviving (e.g. when a `docs/<locale>/about_X.md` is deleted), a new `RemoveOrphanedExternalHelp` task runs in the `Build` chain right after `GenerateExternalHelp` and trims any locale dirs / `about_*.help.txt` files / leftover XMLs whose markdown source has been removed. Cold `GenerateExternalHelp` cost is unchanged at ~550 ms; warm runs drop to ~20 ms (-95%). Concretely, an inner-loop `Invoke-Build -Task Build` (without doc edits) drops from ~3.4 s to ~1.0 s on macOS.
- Cut ~2 s of fixed overhead from every `Invoke-Build` invocation by lazy-loading `BuildHelpers` and dropping the eager `#requires PowerShellGet` directive from `Tools/BuildTools.psm1`. The module previously paid ~555 ms on cold import (PSGet auto-load) plus ~465 ms on `Set-BuildEnvironment` (BuildHelpers auto-import + `git rev-parse` / `git log` / `git branch` introspection) — for every `Invoke-Build` call, regardless of which task ran. The five env vars that actually need git introspection (`BHBranchName`, `BHCommitHash`, `BHCommitMessage`, `BHBuildNumber`, `BHBuildSystem`) are only read by `ShowDebugInfo`, so the heavy work now happens inside that task only via a new `Initialize-BuildEnvironmentInfo` helper. The static path env vars (`BHProjectName`, `BHProjectPath`, `BHModulePath`, `BHPSModulePath`, `BHPSModuleManifest`, `BHBuildOutput`) are populated directly at the top of `JiraPS.build.ps1` from `$PSScriptRoot`, matching what `Set-BuildEnvironment` would have computed. PowerShellGet still auto-loads on demand inside `Install-Dependency`, so `setup.ps1` is unaffected. Measured on macOS: cold `Invoke-Build -Task ?` overhead drops from ~2.4 s to ~225 ms (-90 %); warm overhead drops from ~200 ms to ~30 ms.
- Removed ~30 live HTTP round-trips to `postman-echo.com` from `Tests/Functions/Public/Invoke-JiraMethod.Unit.Tests.ps1`. The previous `Invoke-WebRequest` mocks (both the top-level one and the paging-context one) shelled out to the real `postman-echo.com` and then treated the response as synthetic, which made this "unit" file the slowest in the suite by a wide margin. The mocks now build the response object directly from a `MemoryStream` and mirror postman-echo's `{ args, headers, data, url }` shape offline, so response-inspecting assertions (`$response.headers.'x-fake'`, `$response.data`) continue to hold. `Invoke-JiraMethod.Unit.Tests.ps1` drops from ~9.5 s to ~2 s on macOS (roughly the same win per run on Windows); the full `Invoke-Build -Task Test` drops from ~32 s to ~20 s. The tests also no longer require network connectivity or a reachable `postman-echo.com` to pass.
- Deferred `Get-Help` discovery in `Tests/Help.Tests.ps1` from `BeforeDiscovery` to a per-cmdlet `BeforeAll`, and short-circuited the parameter matrix + MAML parse when not running against the `Release` tree. Previously every `Invoke-Pester ./Tests/Help.Tests.ps1` run eagerly fetched `Get-Help` for all 64 public cmdlets up front (~9 s on macOS) regardless of whether the `Help` / `Parameter` contexts actually ran — in source mode every one of those contexts is `-Skip`'d, so the work was pure waste. The file now runs in ~4.6 s against source (-4.2 s, -48 %) and stays at ~9.7 s against `Release` (same total work, just moved out of discovery).
- Reworked `Initialize-TestEnvironment` (`Tests/Helpers/TestTools.ps1`) so it now ensures the JiraPS module is loaded at the current on-disk version and returns the manifest path. The helper computes a freshness fingerprint over the module directory's `.ps1` / `.psm1` / `.psd1` files (max `LastWriteTimeUtc`) and stashes it inside the freshly imported module's own session state. Subsequent calls short-circuit when the loaded module already matches, so a full Pester run only triggers `Remove-Module` / `Import-Module -Force` once per actual source change instead of once per test file (`~96 forced reimports → ~1` on a clean run, locally `Invoke-Pester ./Tests/` warm-run drops from ~32 s to ~24.5 s on macOS; the win is much larger on Windows where `Import-Module` is several times slower). The cache lives inside the loaded module's scope, so its lifetime is exactly the lifetime of the module instance it describes — a stale-cache / fresh-module mismatch is impossible by construction. All ~117 test files (and the three `BeforeDiscovery` templates) were updated to the new contract: `$script:moduleToTest = Initialize-TestEnvironment` replaces the previous three-line `Initialize-TestEnvironment` + `Resolve-ModuleSource` + `Import-Module -Force` boilerplate. Cache behaviour is covered by `Tests/TestTools.Tests.ps1`.
