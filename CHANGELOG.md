...
- Added Jira Data Center integration test infrastructure (deployment-aware `Tests/Helpers/IntegrationTestTools.ps1`, `Server`/`Cloud` Pester tags on every `Describe` block, `docker-compose.yml`, `Tools/Wait-JiraServer.ps1`, `StartJiraDocker`/`StopJiraDocker` build tasks, and the `.github/workflows/jira_server_ci.yml` workflow) alongside the existing Jira Cloud track, so DC-specific code paths can be exercised end-to-end nightly and on demand.
  ...
  The Server CI job runs nightly (cron `0 5 * * *`) and on `workflow_dispatch` only, **never on PRs** — the ~25 min cold-boot cost (image pull + Maven dep verification + Tomcat / `jira.war` first-time init + Lucene seeding + the parallel Pester suite) is too expensive to gate every PR on, and PR-level Server coverage is already provided by the Server-tagged subset of the per-OS unit suites in `ci.yml` plus the Cloud Smoke job in `integration_tests.yml`. The latest scheduled run reports 147/169 passing; the remaining ~3 failures are AMPS-induced flakes (Versions delete propagation > 60s, Transitions comment-after-transition > 60s, Worklogs read-after-write hang) that don't reflect bugs in JiraPS itself — Cloud sees none of them. Container logs and Pester results are uploaded as artifacts on every run so drift in the upstream image is trivially spottable.
...
- Added a 25 min `timeout-minutes` to the `Run full Server-tagged integration suite` step in `.github/workflows/jira_server_ci.yml`, separate from the job-level 60 min cap. CI run [#24935398326](https://github.com/AtlassianPS/JiraPS/actions/runs/24935398326) showed the parallel test runner (`Tests/Invoke-ParallelPester.ps1`, `ForEach-Object -Parallel`) producing the last per-file `Tests Passed: 10, Failed: 1, Skipped: 1` line at 16:37:44 UTC and then hanging for ~53 min before the job-level timeout cancelled the workflow at 17:30:20 — long enough that neither the orchestrator's `========== TEST SUMMARY ==========` block nor the merged-NUnit XML write block ran, so the artifact-upload + container-teardown steps (gated on `if: always()`) only got a chance to run once GitHub Actions force-killed the step. Capping the step at 25 min gives an honest budget for the actual Pester work (boot is already done in the previous step, the suite reliably completes in <10 min) while killing the orchestrator promptly when `ForEach-Object -Parallel` refuses to wind down — that frees up ~30+ min for the post-step artifact upload and teardown so the `jira-container.log` / `IntegrationTests.xml` artifacts land on a hung run too, and surfaces real hangs in <30 min instead of ~1 hr. The job-level 60 min cap stays as the outer safety net for the boot + provisioning + Pester sequence as a whole.
