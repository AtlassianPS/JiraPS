...
- Updated JiraPS shared standards dependency pins to `AtlassianPS.Standards` `0.1.10` (`Tools/build.requirements.psd1`, `Tools/setup.ps1`, `JiraPS.build.ps1`), with `Tools/setup.ps1` / `Tools/update.dependencies.ps1` resolving the required standards version from `build.requirements`, validating PSGallery availability across runtimes, running NuGet/PSGallery trust preflight only on Windows PowerShell (Desktop), and then using direct `Install-Module`/`Import-Module` bootstrap before delegating to shared commands (`update.dependencies.ps1` now honors `-WhatIf` before any bootstrap side effects).
...
- Pinned JiraPS `setup-powershell` GitHub Action references to the `AtlassianPS.Standards` `v0.1.10` commit SHA, with inline version comments for readability.
...
- **BREAKING**: `Add-JiraIssueLink -IssueLink` now binds to `[AtlassianPS.JiraPS.IssueLinkCreateRequest[]]` via `IssueLinkCreateRequestTransformation` instead of reusing the response/domain `IssueLink` model.
  ...
  `Add-JiraIssueLink` now also treats links as explicit relationships: the `-Issue` anchor parameter was removed and each request must include both `inwardIssue` and `outwardIssue`.
  Added `New-JiraIssueLinkRequest` as a simple helper to build typed request payloads from `-Type`, `-FromIssue`, and `-ToIssue`.
...
- **BREAKING (soft)**: Tightened known URL and timestamp slots on the promoted classes.
  URL-shaped properties such as `RestUrl`, `HttpUrl`, `Self`, `Content`, and `Thumbnail` now surface as `[Uri]` where their shape is known, and issue/comment/worklog/server timestamp slots now use `[DateTimeOffset?]`.
  Scripts that compare these properties as strings should cast explicitly or use `.ToString()`.
...
- **BREAKING**: Renamed the historic `JiraPS.<Type>` PSTypeName for the eight most-used domain types to `AtlassianPS.JiraPS.<Type>`, in line with how every other AtlassianPS module (e.g. ConfluencePS) namespaces its types.
  ...
  The leaf types promoted in this release also use the `AtlassianPS.JiraPS.*` namespace; prefer `$obj -is [AtlassianPS.JiraPS.<Type>]` over PSTypeName string checks for new code.
...
- Promoted the eight most-used JiraPS domain types (`Issue`, `User`, `Project`, `Filter`, `Version`, `Comment`, `Session`, `ServerInfo`) to real .NET classes under the `AtlassianPS.JiraPS.*` namespace, mirroring the long-standing convention used by `ConfluencePS` and other AtlassianPS modules.
  The classes are compiled once per session from the C# source files under `JiraPS/Types/*.cs` via `Add-Type` in the module's `#region Dependencies` block; the type-presence guard keeps re-imports cheap and now detects partially loaded older type assemblies.
  ...
  See the **Removed (Breaking)** section above for the user-visible PSTypeName rename and migration steps. The remaining leaf return types are also promoted in this release; see the separate leaf-type entry above for the affected converter outputs.
...
- **BREAKING (soft)**: Issue-scoped cmdlets now declare `-Issue` (or `-InputObject`, where the historical name is kept) as `[AtlassianPS.JiraPS.Issue]` and use the new `[AtlassianPS.JiraPS.IssueTransformation()]` attribute to coerce the bound value at parameter binding time. Affected cmdlets: `Add-JiraIssueAttachment`, `Add-JiraIssueComment`, `Add-JiraIssueWatcher`, `Add-JiraIssueWorklog`, `Get-JiraIssue` (`-InputObject`), `Get-JiraIssueAttachment`, `Get-JiraIssueComment`, `Get-JiraIssueWatcher`, `Get-JiraIssueWorklog`, `Get-JiraRemoteLink`, `Invoke-JiraIssueTransition`, `Remove-JiraIssue` (`-InputObject`), `Remove-JiraIssueAttachment`, `Remove-JiraIssueWatcher`, `Remove-JiraRemoteLink`, `Set-JiraIssue`, `Set-JiraIssueLabel`.
  ...
  - `Add-JiraIssueLink` no longer uses the historical single-issue anchor input and now requires both `inwardIssue` and `outwardIssue` references in each create request.
...
- **BREAKING (soft)**: The five version-scoped cmdlets that previously accepted `-Version` / `-After` / `-InputObject` / `-InputVersion` as `[Object]`/`[Object[]]` (with a polymorphic `ValidateScript` accepting an `AtlassianPS.JiraPS.Version` PSTypeName, an `[Int]`, or a `[String]` Version name) now declare a real typed parameter and use the new `[AtlassianPS.JiraPS.VersionTransformation()]` attribute to coerce the bound value at parameter binding time. Affected cmdlets:
  ...
  - Unlike the Issue/User/Group transformers, `VersionTransformation` returns alternate JiraPS domain objects unchanged (rather than throwing) so competing `ValueFromPipeline` parameter sets can still bind. `Get-JiraVersion` has sister parameter sets — notably `-InputProject [PSTypeName('AtlassianPS.JiraPS.Project')]` — and a hard transformer throw would block the `Get-JiraProject ... | Get-JiraVersion` pipeline. Truly unrelated values still throw an actionable `ArgumentTransformationMetadataException`.
...
- **BREAKING (soft)**: The four project-scoped cmdlets that previously accepted `-Project` as `[Object]`/`[Object[]]` (with a polymorphic `ValidateScript` accepting an `AtlassianPS.JiraPS.Project` PSTypeName, an `[Int]` ID, or a `[String]` project key) now declare a real typed parameter and use the new `[AtlassianPS.JiraPS.ProjectTransformation()]` attribute to coerce the bound value at parameter binding time. Affected cmdlets:
  ...
  - `ProjectTransformation` also returns alternate JiraPS domain objects unchanged when a competing pipeline parameter set may bind them, but still throws on unrelated values so binding failures stay actionable.
...
- Added a single string-arg convenience constructor to the six identifier-driven `AtlassianPS.JiraPS.*` classes — `Issue(string key)`, `User(string identifier)`, `Project(string key)`, `Group(string name)`, `Filter(string id)`, and `Version(string nameOrId)` — for the recurring stub-from-an-identifier case (`[AtlassianPS.JiraPS.Issue]::new('TEST-1')` instead of `[AtlassianPS.JiraPS.Issue]@{ Key = 'TEST-1' }`).
  ...
  Each identifier-driven class also gains an explicit parameterless ctor (`public Foo() {}`) so the existing `[AtlassianPS.JiraPS.Foo]@{ ... }` hashtable-cast pattern keeps working — declaring a parameterized ctor in C# removes the implicit default, which the 30+ existing call sites depend on; a regression test locks the parameterless ctor into place. `Comment`, `Session`, and `ServerInfo` also now declare explicit parameterless ctors and preserve meaningful `ToString()` behavior without adding misleading string-arg stub ctors. The transformer bodies were reworked to use the new ctors at their stub-construction sites for symmetry with the public surface.
- **Compiled types at import (`Add-Type`)**: Maintainer and host-operator notes. The `AtlassianPS.JiraPS.*` classes compile from the C# source files under `JiraPS/Types/*.cs` on the **first** `Import-Module` in a session (`JiraPS.psm1`, `#region Dependencies`); a type-presence guard keeps re-imports cheap and reports a targeted error if an older partial type assembly is already loaded. **Cold start** pays a small one-time compile cost. **Locked-down hosts** that block just-in-time compilation, or deployments that omit the `.cs` files from the module layout, can fail to load the module. The source must remain compatible with **Windows PowerShell 5.1's bundled CodeDOM compiler (C# 5 only)** — avoid C# 6+ syntax (e.g. `nameof`, interpolated strings, `is Type x` pattern variables) unless the load path is switched to a Roslyn-based compile. **Argument transformers**: when a parameter competes with another pipeline parameter set, return the input unchanged for unrecognized shapes instead of throwing; see `AGENTS.md` → *Argument transformation attributes*.
...
- The six identifier-driven JiraPS classes (`Issue`, `User`, `Project`, `Group`, `Filter`, `Version`) now implement identifier-based equality, hashing, and comparison. Equality is keyed on each type's canonical identifier slot (`Issue.Key`, `User.AccountId ?? Name`, `Project.Key`, `Group.Name`, `Filter.ID`, `Version.ID ?? Name`) so `-eq`, `-contains`, `Group-Object`, hashtable keys, and `Sort-Object -Unique` treat semantically-equivalent stubs as the same value. Identifier-less objects remain distinct and do not collapse under `Sort-Object -Unique`.
- Audited the six argument transformers (`Issue`, `User`, `Group`, `Version`, `Filter`, `Project`) for binder fallthrough across competing pipeline parameter sets. Alternate JiraPS domain objects now pass through unchanged where PowerShell needs to try a different parameter set, while unrelated values still fail fast with actionable transformation errors.
- `Session.ToString()` now falls back to `JiraSession` when `JSessionID` is absent, and `ServerInfo.ToString()` now degrades gracefully when only `Version` or only `DeploymentType` is populated.
