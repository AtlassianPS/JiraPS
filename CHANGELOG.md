...
- `Add-JiraIssueWorklog -Comment` is now wrapped into Atlassian Document Format on Jira Cloud, matching the comment behaviour above. The corresponding read path (`ConvertTo-JiraWorklogitem`, used by `Get-JiraIssueWorklog`) now renders ADF comment payloads back to plain text on the returned `JiraPS.Worklogitem` object so callers see a readable string regardless of deployment. Server / Data Center behaviour is unchanged. (#602)
- `New-JiraIssue -Description` and `Set-JiraIssue -Description` / `-AddComment` are now interpreted as Markdown on Jira Cloud and converted to Atlassian Document Format before being sent, so the create and edit endpoints accept rich-text content on Cloud REST v3. Behaviour on Jira Server / Data Center is unchanged: the values are sent verbatim. The semantics of `Set-JiraIssue -Description ''` (currently a no-op) are unchanged in this release and tracked separately in the backlog. (#602)
- `Invoke-JiraIssueTransition -Comment` is now wrapped into Atlassian Document Format on Jira Cloud, matching the comment behaviour of `Add-JiraIssueComment`. Server / Data Center behaviour is unchanged. (#602)
