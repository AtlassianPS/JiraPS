...
- Added Jira Data Center integration test infrastructure (deployment-aware `Tests/Helpers/IntegrationTestTools.ps1`, `Server`/`Cloud` Pester tags on every `Describe` block, `docker-compose.yml`, `Tools/Wait-JiraServer.ps1`, `StartJiraDocker`/`StopJiraDocker` build tasks, and the `.github/workflows/jira_server_ci.yml` workflow) alongside the existing Jira Cloud track, so DC-specific code paths can be exercised end-to-end on every PR.
  ...
  CI scope is currently limited to the smoke suite (`Tests/Integration/Server.Integration.Tests.ps1`, which proves connectivity, Basic-auth session, and DC-style `name`-based user lookup). The image is `atlas-run-standalone`, which boots a bare Jira platform without `jira-software` / `jira-core` plugins activated — `POST /rest/api/2/project` therefore rejects every project type, so `Wait-JiraServer.ps1` treats fixture-project creation as best-effort and the smoke suite skips the CRUD test with an explicit message when no project is present. The broader `Server`-tagged suite (which assumes a `TEST` project fixture) needs either an image with `jira-software` pre-activated or an SQL/`dbconfig.xml` seeding step and is tracked as a follow-up.
  Run locally with `Invoke-Build -Task StartJiraDocker; $env:CI_JIRA_TYPE='Server'; Invoke-Build -Task TestIntegration -IntegrationTestPath ./Tests/Integration/Server.Integration.Tests.ps1; Invoke-Build -Task StopJiraDocker` (allocate ≥6 GiB to Docker Desktop first; on Apple Silicon set `JIRA_IMAGE_TAG=jira-11-arm64` for the native build).
