...
- `Remove-JiraVersion` now targets the canonical `/rest/api/2/version/{id}` delete endpoint and retries short-lived HTTP 405 responses before surfacing an error. This stabilizes Jira Data Center runs where a freshly-created version can briefly reject immediate deletion even though the same delete succeeds moments later.
