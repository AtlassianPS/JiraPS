...
InModuleScope PSJira {
    ...
    Describe "Add-JiraIssueWatcher" {
        ...
        Mock Invoke-JiraMethod -ModuleName PSJira -ParameterFilter {$Method -eq 'POST' -and $URI -eq "$jiraServer/rest/api/latest/issue/$issueID/watcher"} {
            if ($ShowMockData)
            {
        ...
        It "Adds a Watcher to an issue in JIRA" {
            ...
            $WatcherResult | Should Not BeNullOrEmpty
