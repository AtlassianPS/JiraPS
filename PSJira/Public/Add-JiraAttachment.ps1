function Add-JiraAttachment {
    ...
    process {
        ...
        Write-Debug "[Add-JiraAttachment] Preparing for blastoff!"
        $rawResult = Invoke-JiraMethod @parameter

        if ($PassThru) {
            Write-Debug "[Add-JiraAttachment] -PassThru specified."
            Write-Debug "[Add-JiraAttachment] Converting to custom object"
            ConvertTo-JiraAttachment -InputObject $rawResult

            Write-Debug "[Add-JiraAttachment] Outputting result"
            Write-Output $result
        }
