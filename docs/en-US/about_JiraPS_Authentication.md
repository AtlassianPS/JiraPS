...
_More information on API tokens and how to create one:_
...
_More information on PATs for Data Center:_
_<https://confluence.atlassian.com/enterprise/using-personal-access-tokens-1026032365.html>_
...
New-JiraSession -BearerToken $token
```
...
Once a session is created, you don't need to pass credentials to each command.

### Creating a Session

Choose the method that matches your Jira deployment:

| Deployment | Parameter | Example |
|------------|-----------|---------|
| Jira Cloud | `-ApiToken` + `-EmailAddress` | `New-JiraSession -ApiToken $token -EmailAddress "you@example.com"` |
| Jira Data Center | `-BearerToken` | `New-JiraSession -BearerToken $pat` |
| Either (legacy) | `-Credential` | `New-JiraSession -Credential $cred` |

Once you've created a session, you're done!
You don't need to specify credentials when running other commands - JiraPS will
